...
def generate_filter(fs=650, lowcut=20.0, highcut=250.0, notch_freq=50.0, numtaps=101):
    # Filter parameters
...
def plot_frequency_response(filter, fs=650, lowcut=20.0, highcut=250.0, notch_freq=50.0):
    # Frequency response
...
def block_run(data, filter, windowsize=64, process=True):
    # Ensure filter is an array or list
    if not isinstance(filter, (list, np.ndarray)):
        raise TypeError("Filter must be a list or numpy array")
    ...
    filtered_data = lfilter(filter, 1.0, data)
    # Set first transient_response_length to 0
    filtered_data[:transient_response_length] = 0

    processed_data = filtered_data
    if process:
        for i in range(int(len(data)//windowsize)):
            current_window = processed_data[i*windowsize:(i+1)*windowsize]
            processed_data[i*windowsize:(i+1)*windowsize] = process_data(current_window)
...
if __name__ == '__main__':
    ...
    start = time.time()
    filtered_signal1 = block_run(sensor1_data, filter)
    filtered_signal2 = block_run(sensor2_data, filter)
    end = time.time()
