...
def run(sensor1, sensor2, filter=generate_filter(), return_window=0, threshold=5.0, multiplier=3.0, window_size=300):
    """
    This function takes in the sensor data, filters it, combines it, 
    thresholds it, and then takes the moving average. If return_window is 
    not equal to 0, it will return the mean of the last "return_window" 
    ...
    if return_window != 0:
        value = np.mean(combined_sensor[return_window:])
        return value
